...
public class PlayerStats : MonoBehaviour
{
    ...
    private void LoadCurrenciesFromSave()
    {
        if (SaveSystem.Instance != null)
        {
            ...
            MizuLog.Save($"📥 Currencies loaded: Y={currencyDict[EnumCurrency.yellowBits]}, B={currencyDict[EnumCurrency.blueBits]}, P={currencyDict[EnumCurrency.pinkBits]}, G={currencyDict[EnumCurrency.greenBits]}, XP={currencyDict[EnumCurrency.xpBits]}");
            MizuLog.Save($"🌟 Player loaded: Lv{statsDict[EnumStat.level]}, Exp{statsDict[EnumStat.exp]}");
        }
    }
    ...
    private void CheckLevelUp()
    {
        ...
        while (currentExp >= expRequired)
        {
            ...
            MizuLog.General($"Level Up! Now level {currentLevel} (+1 XpBits)");
