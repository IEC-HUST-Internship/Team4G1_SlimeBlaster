...
public class PlayerCombatArena : MonoBehaviour
{
    ...
    private void OnEnable() 
    {
        ...
        if (playerStats != null)
        {
            ...
            MizuLog.Combat($"🎮 PlayerCombatArena: attackSizeCount = {attackSizeCount}, playerEffect = {playerEffect != null}");
    ...
    private void Update()
    {
        if (!isDead)
        {
            ...
            if (GameTimeElapsed - lastDebugTime >= 10f)
            {
                ...
                MizuLog.Combat($"⏱️ GameTime: {GameTimeElapsed:F0}s / 180s (3min bonus)");
            }
        }
    }
            ...
            if (!isDead && playerStats != null)
            {
                ...
                if (hpLoss > 0)
                {
                    ...
                    if (gameTimeElapsed >= nextIncreaseTime)
                    {
                        ...
                        MizuLog.Combat($"⏱️ HP Loss increased! Now: {playerStats.GetStatValue(EnumStat.hpLossPerSecond)} HP/sec (Stage {currentStage})");
                    }
    ...
    private void Attack()
    {
        ...
        if (isCriticalHit)
        {
            ...
            MizuLog.Combat($"💥 CRITICAL HIT! Damage boosted to {finalDamage} ({critMultiplier:F2}x)");
        }
