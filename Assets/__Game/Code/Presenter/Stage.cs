...
public class Stage : Singleton<Stage>
{
    ...
    public void IncreaseStage()
    {
        if (stage < unlockedStage)
        {
            ...
            MizuLog.General($"Stage increased to: {stage}");
        }
        else
        {
            MizuLog.General($"Cannot increase stage. Maximum unlocked stage is: {unlockedStage}");
        }
    }
    ...
    public void DecreaseStage()
    {
        if (stage > 1)
        {
            ...
            MizuLog.General($"Stage decreased to: {stage}");
        }
        else
        {
            MizuLog.General("Cannot decrease stage below 1");
        }
    }
    ...
    public void SetStage(int newStage)
    {
        if (newStage >= 1 && newStage <= unlockedStage)
        {
            ...
            MizuLog.General($"Stage set to: {stage}");
        }
        else
        {
    ...
    public void UnlockStages(int amount)
    {
        ...
        MizuLog.General($"Unlocked {amount} new stages. Current stage: {stage}, Unlocked: {unlockedStage}, Max: {maxStage}");
    }
